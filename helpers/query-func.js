...
class Query {
    ...
    async viewBudget() {
        try {
            ...
            const depToViewBudget = await inquirer.prompt(question.selectDepQuestion(depList, 'view'));
    ...
    async addDepartment() {
        try {
            const newDep = await inquirer.prompt(question.departmentQuestion());
    ...
    async addRole() {
        try {
            ...
            const newRole = await inquirer.prompt(question.roleQuestion(depList));
    ...
    async addEmployee() {
        try {
            ...
            const newEmployee = await inquirer.prompt(question.employeeQuestion(roleList, managerList));
    ...
    async updateEmployee() {
        try {
            ...
            const [getRole] = await db.promise().query('SELECT * FROM role;');
            const roleList = getRole.map(({ id, title }) => ({ name: title, value: id }));
            const updatedEmployee = await inquirer.prompt(question.updateQuestion(employeeList, roleList));
    ...
    async deleteDep() {
        const [getDep] = await db.promise().query('SELECT * FROM department;');
        const depList = getDep.map(ele => ele.name);
        const depToDelete = await inquirer.prompt(question.selectDepQuestion(depList, 'delete'));
